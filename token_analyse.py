from config import *
import numpy as np
import pandas as pd
from transformers import AutoTokenizer
import matplotlib.pyplot as plt
from data_exploration import ArMI2021, OSACT2022, HSARABIC

if __name__ == '__main__':
    #process data save merged files
    ArMI_2021 = ArMI2021()
    OSACT2022 = OSACT2022()
    HSArabic = HSARABIC()
    merges = [('ArMI_2021',ArMI_2021.df_merge), ('OSACT2022', OSACT2022.df_merge), ('HSArabic', HSArabic.df_merge)]


    for name, df in merges:
        print('\nTokens Analyses: {}'.format(name))
        print('File: {}'.format(name + '_merge.tsv'))

        for transformer in TRANSFORMERS:
            
            print(f'Transformer: {transformer} \n')
            tokenizer = AutoTokenizer.from_pretrained(transformer)
            
            # tokenize text
            df['tokens'] = df['text'].apply(lambda x: tokenizer.tokenize(x))
            # count tokens
            df['number_tokens'] = df['tokens'].apply(lambda x: len(x))

            #print a histogram of tokens
            ax = df['number_tokens'].plot.hist(bins=20, range=(0, 512))
            ax.plot()
            # plt.show()
            plt.savefig(f'{GRAPHICS_PATH}/histogram_{transformer.split("/")[-1]}_{name}.png')

            # print % of tokens covered by each max_len
            print('Percentage of comments under 64 tokens: ',((df.query('number_tokens < 64').size / df.size) * 100))
            print('Percentage of comments under 128 tokens: ',((df.query('number_tokens < 128').size / df.size) * 100))
            print('Percentage of comments under 256 tokens: ',((df.query('number_tokens < 256').size / df.size) * 100))
            print('Percentage of comments under 512 tokens: ',((df.query('number_tokens < 512').size / df.size) * 100))
            
            
            
            
            
            
            
            
# if __name__ == '__main__':
#     #remove old data
#     clear_directory(BASELINE_DATA_PATH)
    
#     #process data save merged files
#     ArMI_2021 = ArMI2021()
#     ArMI_2021.save_tsv(
#                 path=BASELINE_DATA_PATH,
#                 splits=['merge'])
    
#     OSACT2022 = OSACT2022()
#     OSACT2022.save_tsv(
#                 path=BASELINE_DATA_PATH,
#                 splits=['merge'])
    
#     HSArabic = HSARABIC()
#     HSArabic.save_tsv(
#                 path=BASELINE_DATA_PATH,
#                 splits=['merge'])
    
#     # create a list with merged files
#     merged_files = [file for file in os.listdir(BASELINE_DATA_PATH) if 'merge' in file]

#     for file in merged_files:
#         print('\nTokens Analyses: {}'.format(file.split('_')[0]))
#         print('File: {}'.format(file))
#         df = pd.read_csv(BASELINE_DATA_PATH + '/' + file, sep='\t', index_col='tweet_id')

#         for transformer in TRANSFORMERS:
            
#             print(f'Transformer: {transformer} \n')
#             tokenizer = AutoTokenizer.from_pretrained(transformer)
            
#             # tokenize text
#             df['tokens'] = df['text'].apply(lambda x: tokenizer.tokenize(x))
#             # count tokens
#             df['number_tokens'] = df['tokens'].apply(lambda x: len(x))

#             #print a histogram of tokens
#             ax = df['number_tokens'].plot.hist(bins=20, range=(0, 512))
#             ax.plot()
#             # plt.show()
#             plt.savefig(f'{GRAPHICS_PATH}/histogram_{transformer.split("/")[-1]}_{file.split(".")[0]}.png')

#             # print % of tokens covered by each max_len
#             print('Percentage of comments under 64 tokens: ',((df.query('number_tokens < 64').size / df.size) * 100))
#             print('Percentage of comments under 128 tokens: ',((df.query('number_tokens < 128').size / df.size) * 100))
#             print('Percentage of comments under 256 tokens: ',((df.query('number_tokens < 256').size / df.size) * 100))
#             print('Percentage of comments under 512 tokens: ',((df.query('number_tokens < 512').size / df.size) * 100))